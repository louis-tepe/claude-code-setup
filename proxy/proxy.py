...
def _strip_thinking_from_history(data: dict) -> int:
    """Remove foreign thinking blocks (invalid/empty signatures) from history.

    Providers like MiniMax and MiMo emit thinking blocks with `signature: ""`
    which Anthropic rejects with 'Invalid signature in thinking block'.
    Real Anthropic thinking blocks carry cryptographic signatures that MUST
    be preserved so Opus keeps its reasoning memory between turns.

    A block is considered foreign if its signature is missing, None, or empty.
    ...
    for msg in data.get("messages", []):
        ...
        msg["content"] = [
            ...
            if not (
                ...
                and not block.get("signature")  # keep blocks with real signature
            )
        ]
        blocks_removed += original_len - len(msg["content"])
        if not msg["content"]:
